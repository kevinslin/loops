...
- [x] bug: inner loop appears "stuck" in `PR_APPROVED` when PR is approved but not merged. cleanup runs once, then loop polls forever waiting for `mergedAt` without escalating to `NEEDS_INPUT` or marking done. reproduce with `review_status=approved` and `mergedAt=null`; expected behavior: either finish after cleanup (if merge is out of scope) or escalate with clear handoff after a bounded wait.
