...
- PR discovery is file-based via `LOOPS_PR_ARTIFACT_FILE` (run-scoped, set by launcher); default fallback is `/tmp/{current-dir}-devloop-pr`. Inner loop does not scrape PR URLs from model output text.
