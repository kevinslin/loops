...
- Codex turn behavior (`loops/inner_loop.py:601`):
  ...
  - Discovers PR URL from artifact file only (hardcoded path: `/tmp/{sha1(LOOPS_RUN_DIR)}-devloop-pr` when `LOOPS_RUN_DIR` exists, else fallback `/tmp/{current-dir}-devloop-pr`), and clears that artifact before each turn to avoid stale PR reuse.
...
A: If exit code is zero but no PR URL artifact is written to the expected artifact path (`/tmp/{sha1(LOOPS_RUN_DIR)}-devloop-pr`, or fallback `/tmp/{current-dir}-devloop-pr`), the loop requests manual guidance.
...
## Changelog
- 2026-03-01: Switched PR discovery from output scraping to artifact-file reads only, using hardcoded run-scoped path `/tmp/{sha1(LOOPS_RUN_DIR)}-devloop-pr` (fallback `/tmp/{current-dir}-devloop-pr`) with per-turn artifact clearing to prevent stale PR reuse.
