...
- Codex turn behavior (`loops/inner_loop.py:601`):
  ...
  - Discovers PR URL from Codex output (`pr_url`/`pull_request_url`/`url` JSON fields or plain GitHub PR URL text in output).
...
A: If exit code is zero but no PR URL can be extracted from Codex output and no explicit `NEEDS_INPUT` marker is present, the loop requests manual guidance.
...
## Changelog
- 2026-03-01: Restored PR discovery to Codex output parsing and removed artifact-file PR URL discovery.
