...
- Codex turn behavior (`loops/inner_loop.py:601`):
  ...
  - Discovers PR URL from artifact file only (path from `LOOPS_PR_ARTIFACT_FILE`; else `/tmp/{run-dir-name}-devloop-pr` derived from `LOOPS_RUN_DIR`; else fallback `/tmp/{current-dir}-devloop-pr`), and clears that artifact before each turn to avoid stale PR reuse.
...
A: If exit code is zero but no PR URL artifact is written to the expected artifact path (`LOOPS_PR_ARTIFACT_FILE`, or `LOOPS_RUN_DIR`-derived `/tmp/{run-dir-name}-devloop-pr`, or fallback `/tmp/{current-dir}-devloop-pr`), the loop requests manual guidance.
...
## Changelog
- 2026-03-01: Switched PR discovery from output scraping to artifact-file reads only, using run-scoped `LOOPS_PR_ARTIFACT_FILE` (`LOOPS_RUN_DIR`-derived fallback, then `/tmp/{current-dir}-devloop-pr`) with per-turn artifact clearing to prevent stale PR reuse.
